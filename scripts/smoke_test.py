...
def main() -> None:
    ...
    batch = {
        "observation.images.front": torch.rand(2, cfg.n_obs_steps, 3, 64, 64),
        "observation.images.wrist": torch.rand(2, cfg.n_obs_steps, 3, 64, 64),
        ...
        OBS_STATE: torch.randn(2, cfg.n_obs_steps, 10),
    ...
    assert obs_tokens.shape[1] >= cfg.max_image_tokens * cfg.n_obs_steps
    assert fm_actions.shape == (2, cfg.chunk_size, cfg.max_action_dim)
    assert refinement_out["probe"]["failure_probability"].shape == (2,)
